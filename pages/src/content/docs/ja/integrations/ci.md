...
2. **runner に `ocr` をインストール**します。通常は
   `npm install -g @lab-overflow/open-code-review` です。runner は一時的なため、これは実行のたびに発生します。
...
上流のワークフローは
[`examples/github_actions/ocr-review.yml`](https://github.com/Lab-Overflow/open-code-review/blob/main/examples/github_actions/ocr-review.yml)
にあります。
...
- `npm install -g @lab-overflow/open-code-review` で OCR をインストールし、`ocr config set` で設定を書き込み、ブランチ区間モードで中核コマンドを実行します。
...
curl -o .github/workflows/ocr-review.yml \
  https://raw.githubusercontent.com/Lab-Overflow/open-code-review/main/examples/github_actions/ocr-review.yml
```
...
- name: Install OpenCodeReview
  run: npm install -g @lab-overflow/open-code-review@1.0.0
```
...
上流のパイプラインは
[`examples/gitlab_ci/.gitlab-ci.yml`](https://github.com/Lab-Overflow/open-code-review/blob/main/examples/gitlab_ci/.gitlab-ci.yml)
にあります。
...
curl -o .gitlab-ci.yml \
  https://raw.githubusercontent.com/Lab-Overflow/open-code-review/main/examples/gitlab_ci/.gitlab-ci.yml
```
...
```yaml
script:
  - npm install -g @lab-overflow/open-code-review@1.0.0
```
