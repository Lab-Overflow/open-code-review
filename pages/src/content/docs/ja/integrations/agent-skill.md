...
リポジトリには
[`skills/open-code-review/SKILL.md`](https://github.com/Lab-Overflow/open-code-review/blob/main/skills/open-code-review/SKILL.md)
...
これは
[skills registry](https://github.com/Lab-Overflow/open-code-review/blob/main/skills/open-code-review/SKILL.md)
...
> （`npm install -g @lab-overflow/open-code-review` を通じて）`ocr` CLI を自動的にインストールします——[skill が行うこと](#what-the-skill-does)を参照してください。ただし、LLM は事前に設定しておく**必要があります**。skill が代わりに行うことはできず、処理を止めて尋ねます。[設定](../../configuration/)を参照してください。
...
2. **CLI が無ければ自動インストール。** `which ocr` が "NOT INSTALLED" を報告した場合、agent は `npm install -g @lab-overflow/open-code-review` を実行して続行します。ユーザーへの確認は行いません——これは通常のセットアップ手順とみなされます。
